...
class Protocol:
    ...
    def __init__(self, dosing_type, dosing_pattern, dose, T=None, dose_period=None, absorption_rate=None):  # noqa
        ...
        if self.dosing_pattern == 'instantaneous':
            ...
            if type(T) != list:
                raise TypeError('Not correct format for dosing times, please use list')  # noqa
            if len(T) != len(dose):
        ...
        if self.dosing_type == 'subcutaneous':
            ...
            raise KeyError('Not correct dosing type, either intravenous or subcutaneous')  # noqa 

    def create_dose_function(self, t):
        """ Creates dose function

        Create a function that models the way in which the drug enters the system.  # noqa
        ...
        def bump_fn(t, inject_time, height, sharpness, width):
            ...
            height: numeric
                the amount of drug, which translates to the height of the function  # noqa
            sharpness: numeric
            ...
            """
            return height / (1 + np.exp(- sharpness * (t - inject_time))) - height / (1 + np.exp(- sharpness * (t - inject_time - width)))  # noqa
    ...
    def create_subcutaneous_comp_function(self, t, q):
        ...
        Create a function that models the way in which the drug is absorbed by the system.  # noqa
